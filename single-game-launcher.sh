#!/bin/bash
# single-game-launcher.sh - Single game Chromium kiosk launcher for Creation Station Arcade
# Launches directly to a specific game, no menu
...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] Single game mode: $GAME_NAME" >> $LOG_FILE
...
if [ -d "$SOURCE_DIR/.git" ]; then
    ...
    # Quick fetch + update (10s timeout so boot isn't delayed if no network)
    timeout 10s git fetch origin "$BRANCH" >> "$LOG_FILE" 2>&1 || true
    if git rev-parse --verify "origin/$BRANCH" >/dev/null 2>&1; then
        LOCAL=$(git rev-parse HEAD)
        REMOTE=$(git rev-parse "origin/$BRANCH")
        if [ "$LOCAL" != "$REMOTE" ]; then
            echo "[$(date +'%Y-%m-%d %H:%M:%S')] Git: updating source to origin/$BRANCH" >> "$LOG_FILE"
...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] Launching Chromium kiosk with game: $GAME_NAME (window: $WIN_SIZE)" >> $LOG_FILE
"$CHROMIUM_BIN" \
